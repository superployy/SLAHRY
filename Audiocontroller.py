...
class AudioController(object):
    """Controls the playback of audio and the sequential playing of songs."""
    ...
    async def play_song(self, song):
        ...
        if not self.playlist.loop:  # let timer run through if looping
            self.timer.cancel()
            self.timer = utils.Timer(self.timeout_handler)
        ...
        if song.info.title is None:
            ...
            try:
                downloader = yt_dlp.YoutubeDL({
                    'format': 'bestaudio/best', 'title': True,
                    'cookiefile': config.COOKIE_PATH, 'quiet': True,
                ...
                downloader = yt_dlp.YoutubeDL({
                    'title': True, 'cookiefile': config.COOKIE_PATH, 'quiet': True,
                    'extractor_args': {'youtube': {'player_client': ['web']}},
                })
                r = downloader.extract_info(song.info.webpage_url, download=False)
            ...
            song.info.thumbnail = thumbnails[0]['url'] if thumbnails else None
        ...
        source = discord.FFmpegPCMAudio(song.base_url, **ffmpeg_options)
        transformed = discord.PCMVolumeTransformer(source, volume=float(self.volume) / 100.0)

        self.guild.voice_client.play(transformed, after=lambda e: self.next_song(e))

        self.playlist.playque.popleft()

        for queued_song in list(self.playlist.playque)[:config.MAX_SONG_PRELOAD]:
            asyncio.ensure_future(self.preload(queued_song))

    async def process_song(self, track):
        """Adds the track to the playlist and plays it if it is the first song."""
        ...
        def _make_ydl_opts(fallback=False):
            opts = {
        ...
        try:
            downloader = yt_dlp.YoutubeDL(_make_ydl_opts())
            try:
            ...
            downloader = yt_dlp.YoutubeDL(_make_ydl_opts(fallback=True))
            r = downloader.extract_info(track, download=False)
    ...
    async def process_playlist(self, playlist_type, url):
        ...
        if playlist_type == linkutils.Playlist_Types.Spotify_Playlist:
            ...
            for link in links:
                song = Song(linkutils.Origins.Playlist, linkutils.Sites.Spotify, webpage_url=link)
                self.playlist.add(song)
        ...
        if playlist_type == linkutils.Playlist_Types.BandCamp_Playlist:
            ...
            with yt_dlp.YoutubeDL(options) as ydl:
                ...
                for entry in r['entries']:
                    link = entry.get('url')
                    song = Song(linkutils.Origins.Playlist, linkutils.Sites.Bandcamp, webpage_url=link)
                    self.playlist.add(song)
    ...
    async def preload(self, song):
        ...
        await asyncio.wait(
            fs={loop.run_in_executor(executor, down, song)},
            return_when=asyncio.ALL_COMPLETED,
        )

    def search_youtube(self, title):
        """Searches YouTube for the title and returns the first result's URL."""
    ...
    async def prev_song(self):
        """Loads the previous song from history into the queue and plays it."""
        self.timer.cancel()
        self.timer = utils.Timer(self.timeout_handler)
